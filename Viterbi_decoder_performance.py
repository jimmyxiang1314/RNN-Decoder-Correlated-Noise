...
def get_args():
    ...
    parser.add_argument('-noise_type',        choices = ['awgn','pink_noise','t-dist','hyeji_bursty'], default='awgn')
    parser.add_argument('-decoding_type',        choices = ['unquantized', 'hard'], default='unquantized')
...
if __name__ == '__main__':
    ...
    def turbo_compute((idx, x)):
        ...
        received  = corrupt_signal(coded_bits, noise_type =args.noise_type, code_rate = args.code_rate, sigma = test_sigmas[idx],
        ...
        if args.decoding_type=='hard':
            received = np.where(received<0,0,1)
        decoded_bits = cc.viterbi_decode(received.astype(float), trellis1, tb_depth, decoding_type=args.decoding_type)
