...
'''
def corrupt_signal(input_signal, noise_type, code_rate, sigma = 1.0,
    ...
    if noise_type == 'awgn':
        ...
        ini_noise = cn.powerlaw_psd_gaussian(1,[code_rate,data_shape[0]/code_rate])
        noise = sigma * np.swapaxes(ini_noise,0,1).reshape(data_shape[0])
        corrupted_signal = 2.0*input_signal-1.0 + noise
